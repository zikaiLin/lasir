...
for(f in 1:50){
  try({
    ...
    EM <- mvnMixSEMFix_fix$new(outcome= Y_star,
                               ...
                               preSpec_cluster = rep(1, N))
    
    res_EM <- EM$run.EM(loglik_tol=1e-4)
    
    main_coef[,,1] = res_EM$main_basis_coef[,,1] %*% t(eigens_trans$svd$u) 
    ...
    # -------- Testing of SVCM --------- #
    Y_pred_1grp = predict_gp_known_latent(x = cbind(1, X_gp_test), 
                                     z = X_demo_test,
                                     G = G_train,
                                     fixed_coef  = fixed_coef,
                                     main_coef  = main_coef,
                                     gamma_coef = site_coef,
                                     grp_ind = rep(1, length(fold_ind_test))
    )

    mse_y_svcm[f] = mean((Y_test - Y_pred_1grp$y_pred)^2)
