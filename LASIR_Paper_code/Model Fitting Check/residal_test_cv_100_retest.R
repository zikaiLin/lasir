...
# # partition the data into 10 repetition across sites
# # folds = caret::createFolds(1:N, k = 10, returnTrain = T)
# createFolds_sites = function(N, sites, k = 10){
#   # Combine sites with small number of 
#   
#   folds = list()
#   folds_g = list()
#   site_num_less_20 = as.integer(names(table(sites)[which(table(sites) < 20)]))
#   sites[sites %in% site_num_less_20] = 22
#   sites_cat = unique(sites)
#   sites_size = table(sites)
#   
#   n_each_fold = floor(N/k)
#   for(g in sites_cat){
#     sites_ind_g = which(sites == g)
#     subfolds_g = caret::createFolds(sites_ind_g, k = k, returnTrain = T)
#     
#     folds_g[[as.character(g)]] = lapply(subfolds_g, function(x) sites_ind_g[x])
#   }
#   
#   folds = lapply(1:k, function(s){
#     fold_s = unlist(lapply(folds_g, function(x) x[s]))
#     names(fold_s) = NULL
#     fold_s
#   })
#   
...
for(f in 1:50){
  try({
    ...
    Y_pred = predict_gp_known_latent(x = cbind(1, X_gp_test), 
                                     ...
                                     grp_ind = clusters[fold_ind_test])
    
    mse_y_lasir[f] = mean((Y_test - Y_pred$y_pred)^2)
