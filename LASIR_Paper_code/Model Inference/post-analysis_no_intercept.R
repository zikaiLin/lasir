# rm(list = ls())
# load("/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section2-EM-run/result_p12_rho05_varDF_noIntercept.RData")
input_dat <-
...
main_effect_coef <- readRDS("/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section3-post-process/est_main_effect_no_intercept.rds")
pval_est_main_effect <- readRDS("/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section3-post-process/pval_est_main_effect_no_intercept.rds")
sd_est_main_effect <- readRDS("/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section3-post-process/est_sd_main_effect_no_intercept.rds")
...
pval_dir = "/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section3-post-process/alpha_pval_no_intercept"
coef_dir = "/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section3-post-process/alpha_coef_no_intercept"
...
for(j in 1:length(names_X)){
  ...
  for(k in  1:nK){
    ...
    slot(a1_img_sig,".Data")[input_dat$vox_eff] =  pval_est_main_effect[j,,k] 
    # slot(a1_img_sig,".Data")[!input_dat$vox_eff] = 1
    
    alpha_img_list_pval[[j]][[k]] = a1_img_sig
    filename1_sig = file.path(pval_dir, sprintf("alpha_%s_pval_no_intercept_k%d", filename, k))
...
for(j in 1:(J+1)){
  ...
  for(k in 1:nK){
    ...
    slot(a1_img,".Data")[input_dat$vox_eff] =  main_effect_coef[j,,k]
    
    # correct p-value
    if(j>1){
      filename1_sig = file.path(pval_dir, sprintf("alpha_%s_pval_no_intercept_k%d", filename, k))
      p_thres = as.numeric(system(sprintf("/usr/local/fsl/bin/fdr -i %s -q 0.05 ", filename1_sig), intern = T)[2])
    ...
    filename1_sig_coef = file.path(coef_dir, sprintf("alpha_%s_coef_no_intercept_k%d", filename, k))
...
for(j in 1:(J+1)){
  ...
  for(k in 1:nK){
    ...
    slot(a1_img,".Data")[input_dat$vox_eff] =  (main_effect_coef[j,, k])/(sd_est_main_effect[j,, k])
    ...
    filename1_sig_coef = file.path(coef_dir, sprintf("alpha_%s_tval_no_intercept_k%d", filename, k))
    oro.nifti::writeNIfTI(alpha_img_list_sig[[j]][[k]], filename=filename1_sig_coef)
  }
}
...
for(j in 1:(J+1)){
  ...
  for(k in 1:nK){
    try({
      ...
      sig_voxel_counts = table(power.264.img[power.264.img >0 & !is.nan(alpha_img_list_sig[[j]][[k]])])
      sig_rois = as.integer(names(sig_voxel_counts)[sig_voxel_counts>0]) # roi with sig voxels >= 10
      sum_table = brainGraph::power264[sig_rois,]
      sum_table$ROI = sig_rois
      if(nrow(sum_table) == 0) next()
      
      sum_table$roi_sign = sum_table$vox_count = rep(NA, nrow(sum_table))
      for(roi in 1:length(sum_table$roi_sign)){
        ...
        sum_table$vox_count[roi] = sig_voxel_counts[which(names(sig_voxel_counts) == as.character(ROI))]
      }
      
      alpha_sig_regions[[names_X[j]]][[k]]$all_roi = sum_table
      
      alpha_sig_regions[[names_X[j]]][[k]]$networks = unique(sum_table[,"network"])
...
save(list = "alpha_sig_regions", file = "/Volumes/easystore/Dropbox/U-Mich/Research/ABCD/EM_RDA/section3-post-process/alpha_sig_regions_rho005_p14_no_intercept.RData")
...
############################################################################
grp_ind_fct = relevel(factor( mod$cluster), ref = "1")
library(nnet)
...
# 
# for(g in 1:21){
#   sites_count =  table( mod$cluster[input_dat$G == g])
#   for(k in 1:nK){
...
# site_cluster1_random_efect = apply( m6$random_coef[site_cluster1, ], 2, mean)
# site_cluster2_random_efect = apply( m6$random_coef[site_cluster2, ], 2, mean)
# site_cluster3_random_efect = apply( m6$random_coef[site_cluster3, ], 2, mean)
# site_cluster4_random_efect = apply( m6$random_coef[site_cluster4, ], 2, mean)
# 
# 
# # Write nifti image
# library(oro.nifti)
...
#   grp_ind =  mod$cluster,
#   alpha_coef_post =  m6$fixed_coef,
#   gamma_post_all =  m6$random_coef[input_dat$G,],
#   main_coef =  m6$fixed_main_coef
# )


nK    = length( m6$model_best@size)

Y_pred = predict_gp(x = cbind(1, input_dat$X_g), z = cbind(1, input_dat$X_demogrphic), G = input_dat$G,
                    main_coef =  m6$fixed_main_coef,
                    alpha_coef =  m6$fixed_coef,
                    gamma_coef =  m6$random_coef,
                    grp_ind =  mod$cluster)
